...
Example: `36|muse|5a7e0113-…|OLI-1211|2026-09-11T18:50:11Z`

`board.sh` renders it. `retire.sh` records then deletes that N.

## Recorded run file

`automation-super-run-logs/<dir>/<NNN>-<TICKET>.md` produced by `record.sh`.
Fields: model, created, result id, Linear URL, session, drive job, result,
diagnose job, diagnosis + summary, pipeline `complete`|`incomplete`, optional
operator note, webhook log.

Index row appended to `automation-super-run-logs/index.tsv`.
...
- **`inFlight` leak:** dispatcher only claims when a process-local counter
  `< jobs`. After hours it stuck high → pending forever, `jobs in flight: 0`.
  Restart automation-server. Prefer deriving in-flight from
  `automation_jobs.status='running'`.
...
- **Free Muse:** rate limit at 4-wide; daily IP quota; image-upload 400
  after many screenshots. Paid Muse from run 020 onward.
...
On the branch that added it, `./automation-server --jobs N` is how many
drive+diagnose jobs the dispatcher runs at once. `./qemu-server --max-jobs`
and `./automation-client --max-jobs` are the reserve caps. Confirm
`./automation-server --help` on the current checkout — master may not have
`--jobs` yet; then dispatcher concurrency is 1 and you cannot 8-wide.
