...
- Packages today: `@oligarchy/env` (what a process is given from outside), `@oligarchy/async` (this document's lifetime), `@oligarchy/jarl_ext` (`repeat`, a jarl function called again while it fails), `@oligarchy/driver` (the first app, flags only). A package is source-first: `exports` point at `.ts`, no build step, no `dist`.
...
- **Retries.** v1 uses `Effect.retry(Schedule.recurs(2))`: three tries, then a line (`packages/jobs/src/close.ts`, `packages/jobs/src/ready.ts`, the automation server's worker). `@oligarchy/jarl_ext`'s `repeat` calls a jarl function again up to a count, and an error filter that returns false stops and returns that error. `@oligarchy/async`'s `retry` is that plus a delay and a service: it waits on the clock and returns `Closed` instead of trying again once the service has closed.
