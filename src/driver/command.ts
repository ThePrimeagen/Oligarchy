...
export const makeDriverCommand = <E, R>(
  ...
  Command.make(
    ...
    Command.withDescription(
      "Run the harness loop for one prompt and one model: the commands the model asks for, a debug log of every step, a started session marked running, and intent start and end around each guest action",
    ),
    EnvFile.withEnvFile,
  );
