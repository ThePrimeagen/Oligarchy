...
const flags = {
  ...
  prompt: Flag.string("prompt").pipe(
    ...
    Flag.withDescription("The test definition"),
  ),
  proof: Flag.string("proof").pipe(
    Flag.withSchema(Schema.NonEmptyString),
    Flag.withDescription("The proof that closes the test"),
  ),
...
export const makeDriverCommand = <E, R>(
  ...
  Command.make(
    ...
    Command.withDescription(
      "Run the harness loop for one prompt and one model: each reply is one drive tool call, a reason, whether the step completes, and a typed action; a debug log of every step; a started session marked running; and intent start and end around each guest action",
    ),
    EnvFile.withEnvFile,
  );
