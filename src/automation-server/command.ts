...
export const makeAutomationServerCommand = <RServe>(server: AutomationServer<RServe>) =>
  Command.make(
    ...
    Command.withDescription(
      "The automation server: POST /linear verifies a signed Linear webhook and enqueues drive or diagnose jobs, then dispatches them to live automation clients; every thirty seconds the check kicks off one new job per live automation client and never more, moving a ticket unchanged for ninety seconds from Backlog to Automation Needed and queuing it, or else queuing one unchanged ticket in Automation Needed or Needs Review the way that webhook would have queued it; dispatch launches jobs one reservation at a time, round robin onto a client whose reserve succeeds, and does not start the next job until that response is back; a client that already holds a reservation answers 503 and the next client is asked; POST /abort closes a pending job or stops a running one",
    ),
  );
