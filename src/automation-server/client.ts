...
export const run = Effect.fn("run")(function* (url: string, prompt: string, model: string) {
  ...
  return yield* client.Runs.run({ payload: Contract.RunBody.make({ prompt, model }) }).pipe(
    Effect.catch((error) => {
