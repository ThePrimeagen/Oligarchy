...
// minutes of that and the next peer's heartbeat deletes the row, a row nobody ever claimed
// counting from created_at. stats and heartbeat_at are null together, for a row an operator
// added that no server has claimed. type says what kind of server the row is, so a reverse
// proxy lists its own kind; every writer names it, and the default is what the migration
...
// the stable handle a job stores when it is claimed; url remains the key a heartbeat upserts
