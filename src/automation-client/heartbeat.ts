...
// thirty seconds, then forgets every server silent for ten minutes, one info line per row, so a
// killed server leaves the fleet on its peers' word. Its own row is written first, so a client
// back from a long outage keeps its generation rather than deleting itself. A write or the
// cleanup that fails is one error line; the rest of the tick and the next tick still run. A
