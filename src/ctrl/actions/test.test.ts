...
import { createExperiment, printTestDefinitions, selectTestDefinitions } from "./test.ts";
import { linearTicketDescription } from "../../linear.ts";
...
describe("selectTestDefinitions happy path", () => {
  it("returns every definition ordered by name", async () => {
    const rows = await selectTestDefinitions(testDatabase([install, terminal]).db);
    assert.deepEqual(rows, [install, terminal]);
  });

  it("returns the named definition", async () => {
    const rows = await selectTestDefinitions(testDatabase([install, terminal], [terminal]).db, "Open a terminal");
    assert.deepEqual(rows, [terminal]);
  });
});

describe("selectTestDefinitions unhappy path", () => {
  it("rejects a name that matches no definition", async () => {
    await assert.rejects(() => selectTestDefinitions(testDatabase([install], []).db, "missing"), {
...
describe("printTestDefinitions happy path", () => {
  it("prints one name per line", () => {
    ...
    printTestDefinitions([install, terminal], false);
    assert.deepEqual(
      log.mock.calls.map((call) => call.arguments),
      [["Install Omarchy"], ["Open a terminal"]],
    );
  });

  it("prints every field of every definition as JSON", () => {
    const log = mock.method(console, "log", () => undefined);
    printTestDefinitions([install, terminal], true);
...
describe("createExperiment happy path", () => {
  it("creates the run, pending results, and one Linear ticket per definition", async () => {
    const definitions = [install, terminal];
    ...
    assert.deepEqual(
      result.experiment.tests.map((test) => test.id),
  ...
  it("creates the run, one pending result, and one Linear ticket for a named definition", async () => {
    const { db, inserts } = testDatabase([install, terminal], [install]);
    mock.method(console, "error", () => undefined);
    mock.method(globalThis, "fetch", async (_input: Parameters<typeof fetch>[0], init: Parameters<typeof fetch>[1]) => {
      const parsed = await parseLinearRequest(init);
      if (parsed.query.includes("teams(first: 1)")) {
...
describe("createExperiment unhappy path", () => {
  ...
  it("rejects a name that matches no test definition", async () => {
    const { db, inserts, updates } = testDatabase([install], []);
    mock.method(globalThis, "fetch", async () => assert.fail("Linear should not be called"));
  ...
  it("names every ticket created, including one whose description failed", async () => {
    const { db, updates } = testDatabase([install, terminal]);
    let created = 0;
    mock.method(globalThis, "fetch", async (_input: Parameters<typeof fetch>[0], init: Parameters<typeof fetch>[1]) => {
