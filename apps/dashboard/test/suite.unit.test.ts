...
import { bundledPrompts, createTestSuiteRun, SuiteRequestError } from "../src/suite.ts";
...
describe("create test-suite-run runner", () => {
  ...
  it("opens nothing for a body it refuses (unhappy)", async () => {
    ...
    const refused = createTestSuiteRun(env, env.HYPERDRIVE.connectionString, { iso: ISO }, run);
    await expect(refused).rejects.toBeInstanceOf(SuiteRequestError);
    await expect(refused).rejects.toMatchObject({
      _tag: "SuiteRequestError",
      name: "@oligarchy/dashboard/suite/SuiteRequestError",
      message: "iso, version and serverUrl are required",
    });
