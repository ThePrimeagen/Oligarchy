...
type Fixture = {
  readonly stores: ReturnType<typeof TestingStores.fakeStores>;
...
const fixture = (): Fixture => ({
  stores: TestingStores.fakeStores(),
...
describe("POST /linear", () => {
    ...
    () =>
      Effect.gen(function* () {
        ...
        expect(TestingLog.texts(fixed.log)).toEqual([
          "linear webhook queued diagnose; Needs Review",
        ]);
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        ...
        expect(TestingLog.texts(fixed.log)).toEqual([
          "linear webhook ignored; drive already running",
        ]);
        expect(labeled(fixed)).toEqual([]);
      }),
  );
  ...
  it.effect("records In Progress without enqueueing", () =>
    Effect.gen(function* () {
      ...
      expect(fixed.stores.automation.jobs).toEqual([]);
      expect(TestingLog.texts(fixed.log)).toEqual(["linear webhook recorded; In Progress"]);
      expect(labeled(fixed)).toEqual([]);
    }),
  );
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        ...
        expect(fixed.stores.automation.jobs).toEqual([]);
        expect(TestingLog.texts(fixed.log)).toEqual(["linear webhook recorded; Automation Needed"]);
        expect(labeled(fixed)).toEqual([]);
      }),
  );
});
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        ...
        expect(TestingLog.texts(fixed.log)).toEqual([`aborted drive; ${CLIENT_URL}`]);
        expect(fixed.linear.calls).toEqual([
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        ...
        expect(TestingLog.texts(fixed.log)).toEqual(["aborted pending drive"]);
        expect(fixed.linear.calls).toEqual([
          { method: "clearReady", identifier: TICKET },
          { method: "moveToAborted", identifier: TICKET },
        ]);
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        ...
        expect(TestingLog.texts(fixed.log)).toEqual(["aborted pending diagnose"]);
...
describe("POST /abort refusals", () => {
  ...
  it.effect("500 when the client fails, and the job stays running", () =>
    Effect.gen(function* () {
      ...
      expect(TestingLog.texts(fixed.log).filter((text) => text.startsWith("JobNotFound"))).toEqual(
        [],
      );
      expect(fixed.linear.calls).toEqual([]);
    }),
