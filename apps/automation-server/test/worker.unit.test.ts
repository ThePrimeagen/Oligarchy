...
type ResultStatus = TestingStores.FakeTestStore["results"][number]["status"];
...
type JobStatus = TestingStores.FakeAutomationStore["jobs"][number]["status"];
...
const seedDiagnose = (automation: TestingStores.FakeAutomationStore, resultId = RESULT_ID) => {
...
const closing = (tests: TestingStores.FakeTestStore, status: ResultStatus = "passed") =>
  Effect.sync(() => {
...
type Harness = {
  ...
  readonly diagnosis: TestingStores.FakeDiagnosisStore;
...
const harness = (
  ...
  diagnosis: TestingStores.fakeDiagnosisStore(),
  tests: TestingStores.fakeTestStore(),
...
const sentryErrors = (log: TestingLog.FakeLog) =>
...
describe("dispatch happy path", () => {
  ...
  it.effect("a reserved diagnose does not start /run until Linear is In Review", () =>
    Effect.gen(function* () {
      ...
      expect(http.requests.map((request) => request.url)).toEqual([`${URL}/reserve`]);
      expect(TestingLog.texts(fixed.log).some((text) => text.startsWith("dispatching"))).toBe(
        false,
      );
  ...
  it.effect("round robin wraps, so a client that already took a job can take the next", () =>
    Effect.gen(function* () {
      ...
      expect(
        TestingLog.texts(fixed.log).filter((text) => text.startsWith("dispatching drive")),
      ).toEqual([
        `dispatching drive; ${URL}; ${MODEL}`,
...
describe("a drive that returns with its result still open", () => {
  ...
  it.effect("a result lookup that fails after /run errors the job (unhappy)", () => {
    ...
    let lookups = 0;
    const held: { tests: TestingStores.FakeTestStore | undefined } = { tests: undefined };
    ...
    () =>
      Effect.gen(function* () {
        for (const status of ["failed", "errored", "aborted", "timed_out"] as const) {
          ...
          expect(TestingLog.texts(fixed.log)).toEqual(["diagnose aborted"]);
          expect(erroredMoves(fixed.linear)).toEqual([]);
...
describe("dispatch unhappy path", () => {
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        ...
        expect(TestingLog.texts(fixed.log)).toEqual([`dispatching drive; ${URL}; ${MODEL}`]);
        expect(sentryErrors(fixed.log)).toEqual([]);
      }),
  );
    ...
    () =>
      Effect.gen(function* () {
        ...
        const fs = FileSystem.layerNoop({
          readFileString: (path) => Effect.fail(TestingFs.permissionDenied("open", path)),
        });
  ...
  it.effect("a result with no Linear ticket errors after claim and does not POST", () =>
    Effect.gen(function* () {
      ...
      expect(TestingLog.texts(fixed.log)).toEqual(["drive errored; no Linear ticket"]);
      expect(fixed.linear.calls.filter((call) => call.method === "clearReady")).toEqual([]);
    }),
  );
  ...
  it.effect("a job already closed by abort does not die when dispatch finishes", () =>
    Effect.gen(function* () {
      ...
      expect(
        TestingLog.texts(fixed.log).some((text) => text.includes("dispatch tick failed")),
      ).toBe(false);
    }),
  );
  ...
  it.effect("a 503 from every client leaves the job pending and does not POST /run", () =>
    Effect.gen(function* () {
      ...
      yield* start(fixed, http.layer);
      for (let i = 0; i < 200; i++) {
        if (TestingLog.texts(fixed.log).includes("deferred; at capacity")) {
          break;
        }
        yield* Effect.yieldNow;
      }
      ...
      expect(TestingLog.texts(fixed.log)).toEqual(["deferred; at capacity"]);
      expect(sentryErrors(fixed.log)).toEqual([]);
      expect(fixed.linear.calls.filter((call) => call.method === "clearReady")).toEqual([]);
      expect(fixed.log.lines[0]?.agentId).toBe(TICKET);
  ...
  it.effect("a 503 from every client after a success leaves the next job pending", () =>
    Effect.gen(function* () {
      ...
      expect(http.requests.filter((request) => request.url.endsWith("/run"))).toHaveLength(1);
      expect(TestingLog.texts(fixed.log)).toContain("deferred; at capacity");
  ...
  it.effect("a resume drive posts the run's iso and a 409 leaves the job pending", () =>
    Effect.gen(function* () {
      ...
      yield* start(fixed, http.layer);
      for (let i = 0; i < 200; i++) {
        if (TestingLog.texts(fixed.log).includes("deferred; setup needed")) {
          break;
        }
        yield* Effect.yieldNow;
      }
      ...
      expect(fixed.automation.jobs[0]?.status).toBe("pending");
      expect(http.requests).toHaveLength(1);
      expect(TestingLog.texts(fixed.log)).toEqual(["deferred; setup needed"]);
      expect(sentryErrors(fixed.log)).toEqual([]);
    }),
  );
  ...
  it.effect("an unexpected failure on every client is logged and the job stays pending", () =>
    Effect.gen(function* () {
      ...
      expect(fixed.linear.calls).toEqual([]);
      expect(TestingLog.texts(fixed.log).some((text) => text.includes("drive failed"))).toBe(false);
      expect(TestingLog.texts(fixed.log).some((text) => text.includes("drive errored"))).toBe(
        false,
      );
    }),
  );
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        ...
        expect(TestingLog.texts(fixed.log)).toContain(`drive errored; ${RESTARTED}`);
      }),
  );
...
): TestingStores.FakeAutomationStore => ({
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        ...
        expect(TestingLog.texts(fixed.log).filter((text) => text === "drive aborted")).toHaveLength(
          2,
        );
        expect(
          fixed.linear.calls
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        ...
        expect(TestingLog.texts(fixed.log)).not.toContain("drive aborted");
        expect(fixed.linear.calls).toEqual([
          { method: "moveToInProgress", identifier: TICKET },
          { method: "moveToInProgress", identifier: TICKET_B },
        ]);
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        ...
        expect(TestingLog.texts(fixed.log)).toContain("drive aborted");
      }),
  );
  ...
  it.effect(
    ...
    () => {
      ...
      return Effect.gen(function* () {
        ...
        expect(sentTo(http, "/abort")).toEqual([]);
        expect(TestingLog.texts(fixed.log)).toContain("drive completed");
      });
    },
  );
});
...
// client is what the worker meets over HTTP: six slots, a /run that holds its slot until the run
// ends, and an /abort that frees it, answers 404 for a ticket it does not hold, or fails when it
// cannot stop the run. The automation server that died is only what it left behind: six running
// rows, and the six drives the automation client still holds for it.
const MAX_JOBS = 6;
...
const RunsBody = Schema.fromJsonString(
  Schema.Struct({ ticket: Schema.String, prompt: Schema.optionalKey(Schema.String) }),
);
const decodeRunsBody = Schema.decodeUnknownSync(RunsBody);

type SixJobClient = {
  // Each ticket holding a slot, with its run's prompt once /run came, in the order reserved.
  readonly held: Map<string, string | undefined>;
  // Every /run, in order.
  readonly started: Array<string>;
  // A run ending on its own: its slot is free, and an abort for it is a 404.
  readonly finish: (ticket: string) => void;
  readonly http: Layer.Layer<HttpClient.HttpClient>;
};

const sixJobClient = (options: { readonly stuck?: boolean } = {}): SixJobClient => {
  const held = new Map<string, string | undefined>();
  const started: Array<string> = [];
  const http = TestingHttp.respondWith((request, url) => {
    const body = decodeRunsBody(
      request.body._tag === "Uint8Array" ? new TextDecoder().decode(request.body.body) : "{}",
    );
    switch (url.pathname) {
      case "/reserve":
        if (held.size >= MAX_JOBS) {
          return TestingHttp.json({ error: `at capacity: max-jobs is ${String(MAX_JOBS)}` }, 503);
        }
        held.set(body.ticket, undefined);
        return TestingHttp.json({ ok: "true" });
      case "/run":
        held.set(body.ticket, body.prompt);
        started.push(body.ticket);
        // A run answers when it ends, and these do not end while the test runs.
        return Effect.never;
      case "/abort":
        if (!held.has(body.ticket)) {
          return TestingHttp.json({ error: `unknown session "${body.ticket}"` }, 404);
        }
        if (options.stuck === true) {
          return TestingHttp.json({ error: "kill EPERM" }, 500);
        }
        held.delete(body.ticket);
        return TestingHttp.json({ ok: "true" });
    }
    return TestingHttp.json({ error: "not found" }, 404);
  });
  return {
    held,
    started,
...
// connections dropped with it; the automation client keeps each run going.
const leftBehind = (client: SixJobClient, left: ReadonlyArray<string>) =>
  Effect.gen(function* () {
    ...
    yield* until(
      Effect.sync(() => left.every((ticket) => client.held.get(ticket) !== undefined)),
      "every run reached the automation client",
    );
...
const runningRows = (automation: TestingStores.FakeAutomationStore) =>
...
const countingRunning = (automation = TestingStores.fakeAutomationStore()) => {
...
// The prompt of each run the automation client still holds, in the order reserved.
const livePrompts = (client: SixJobClient) => [...client.held.values()];
...
const aMinuteOfTicks = (fixed: Harness) =>
  Effect.gen(function* () {
    const turnedAway = () =>
      TestingLog.texts(fixed.log).filter((text) => text === "deferred; at capacity").length;
...
describe("an automation client with six jobs across an automation server restart", () => {
  it.effect(
    ...
    () => {
      ...
      return Effect.gen(function* () {
        ...
        // After the automation server died, three runs ended and nobody closed their rows: two
        // drivers closed their results first, and one quit with its result open.
        for (const ticket of LEFT.slice(3)) {
          client.finish(ticket);
        }
        const clientId = seedLiveClient(fixed.servers);
        ...
        yield* eventually(
          () => client.started.length === LEFT.length + counted.afterEachWrite.length,
          "every drive written running was run",
        );
        ...
        // The automation client stopped the three still driving; the one that quit it no longer
        // held, and answered 404.
        expect(LEFT.filter((ticket) => client.held.has(ticket))).toEqual([]);
        ...
        expect(livePrompts(client)).toEqual(WAITING.slice(0, 6).map(promptOf));
        expect(client.held.size).toBe(MAX_JOBS);
  ...
  it.effect(
    ...
    () => {
      ...
      const fixed = harness(TestingLinear.fakeLinear(), counted.automation);
      return Effect.gen(function* () {
        const client = sixJobClient({ stuck: true });
        yield* leftBehind(client, LEFT);
        const clientId = seedLiveClient(fixed.servers);
        ...
        expect(livePrompts(client)).toEqual(LEFT.map(promptOf));
        expect(client.held.size).toBe(MAX_JOBS);
      });
    },
  );
