...
import * as Backlog from "../src/backlog.ts";
...
const tracked = (log: TestingLog.FakeLog): ReadonlyArray<string> =>
  TestingLog.texts(log).filter((text) => text.includes(TRACKING));
const acted = (log: TestingLog.FakeLog): ReadonlyArray<string> =>
  TestingLog.texts(log).filter((text) => !text.includes(TRACKING));
const errors = (log: TestingLog.FakeLog): ReadonlyArray<TestingLog.Line> =>
  log.lines.filter((line) => line.level === "error");

const trackingLine = (text: string): TestingLog.Line => ({
  level: "info",
  text,
  location: "automation",
  agentId: "automation",
