...
import * as Child from "../src/child.ts";
...
describe("Sessions.run happy path", () => {
  ...
  it.effect("a diagnose with no oligarchy.json fails and spawns nothing (unhappy)", () => {
    ...
    return Effect.gen(function* () {
      ...
      Effect.provide(
        layer(spawner, MAX_JOBS, qemuOk(), qemuRelinquishOk(), TestingLog.fakeLog(), false),
      ),
    );
  });
  ...
  it.effect("a diagnose that outlives the ceiling names opencode, not the driver", () => {
    ...
    return Effect.gen(function* () {
      ...
      const running = yield* Effect.forkChild(
        Effect.flip(sessions.run(TICKET, "diagnose the session")),
      );
  ...
  it.effect(
    ...
    () => {
      ...
      return Effect.gen(function* () {
        ...
        const running = yield* Effect.forkChild(Effect.flip(sessions.run(TICKET, "do the work")));
        for (let i = 0; i < 100 && spawner.spawned[0] === undefined; i++) {
          yield* Effect.yieldNow;
        }
...
describe("Sessions.abort happy path", () => {
  ...
  it.effect("escalates to SIGKILL when the CLI ignores SIGTERM", () => {
    const spawner = TestingSpawner.fakeSpawner(() => ({ ignoreTerm: true }));
    return Effect.gen(function* () {
      const sessions = yield* Sessions.Sessions;
      yield* sessions.reserve(TICKET, "drive");
      const running = yield* Effect.forkChild(sessions.run(TICKET, "do the work"));
  ...
  it.effect(
    ...
    () => {
      ...
      return Effect.gen(function* () {
        ...
        yield* sessions.reserve(TICKET, "drive");
        yield* sessions.reserve(OTHER, "drive");
        const stuck = yield* Effect.forkChild(sessions.run(TICKET, "stuck"));
        const other = yield* Effect.forkChild(sessions.run(OTHER, "other"));
        for (let i = 0; i < 100 && spawner.spawned.length < 2; i++) {
          yield* Effect.yieldNow;
        }
        expect(spawner.spawned).toHaveLength(2);
...
describe("capacity", () => {
  ...
  it.effect(
    ...
    () => {
      ...
      return Effect.gen(function* () {
        ...
        expect(yield* sessions.jobs).toBe(0);
        expect(spawner.spawned).toHaveLength(1);
        expect((yield* Effect.flip(sessions.run(TICKET, "again")))._tag).toBe("BadRequest");
      }).pipe(Effect.provide(layer(spawner, 1)));
    },
  );
  ...
  it.effect("a run that exited, zero or not, frees its slot", () => {
    const exits = [0, 1];
    const spawner = TestingSpawner.fakeSpawner(() => ({ exitCode: exits.shift() ?? 0 }));
    return Effect.gen(function* () {
  ...
  it.effect("an interrupted run frees its slot once its child is gone", () => {
    ...
    let spawns = 0;
    const spawner = TestingSpawner.fakeSpawner(() => (++spawns === 1 ? {} : { exitCode: 0 }));
    return Effect.gen(function* () {
      const sessions = yield* Sessions.Sessions;
      yield* sessions.reserve(TICKET, "drive");
      const running = yield* Effect.forkChild(sessions.run(TICKET, "first"));
...
describe("reservation expiry", () => {
  it.effect(
    ...
    () => {
      ...
      return Effect.gen(function* () {
        ...
        expect(yield* Effect.flip(sessions.run(TICKET, "late"))).toMatchObject({
          _tag: "BadRequest",
          message: "no reservation",
        });
        yield* sessions.reserve(OTHER, "drive");
  ...
  it.effect("a diagnose reservation unused for ten minutes expires without asking QEMU", () => {
    ...
    return Effect.gen(function* () {
      ...
      expect(yield* sessions.jobs).toBe(0);
      expect(qemu).toEqual([]);
      expect(TestingLog.texts(log)).toEqual(["reservation expired; unused for 10 minutes"]);
    }).pipe(Effect.provide(layer(spawner, 1, reserveQemu, relinquishQemu, log)));
  });
  ...
  it.effect(
    ...
    () => {
      ...
      return Effect.gen(function* () {
        ...
        expect(givenBack).toEqual([TICKET]);
        expect(TestingLog.texts(log)).toEqual(["reservation expired; unused for 10 minutes"]);
        expect((yield* Effect.flip(sessions.run(TICKET, "late")))._tag).toBe("BadRequest");
      }).pipe(Effect.provide(layer(spawner, 1, qemuOk(), relinquishQemu, log)));
    },
  );
  ...
  it.effect("expiry is per reservation: a younger one stays when an older one goes", () => {
    ...
    return Effect.gen(function* () {
      ...
      yield* sessions.run(OTHER, "still mine");
      expect(spawner.spawned[0]?.command).toBe(OpenCode.BIN);
