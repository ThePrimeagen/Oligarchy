...
import { byCommand, type FakeSpawner, fakeSpawner } from "@oligarchy/testing/spawner";
import {
  ABORT_ENV,
  ago,
...
const storesLayer = (
  scripted: Scripted = {},
  actions = TestingStores.fakeActionStore(),
  tests = TestingStores.fakeTestStore(),
  logs = TestingStores.fakeLogStore(),
) =>
  Layer.mergeAll(
...
type Extra = {
  ...
  readonly actions?: TestingStores.FakeActionStore;
  readonly tests?: TestingStores.FakeTestStore;
  readonly logs?: TestingStores.FakeLogStore;
  ...
  Run.run.pipe(
    Effect.provide(
      Layer.mergeAll(
        storesLayer(
          scripted,
          extra.actions ?? TestingStores.fakeActionStore(),
          extra.tests ?? TestingStores.fakeTestStore(),
          extra.logs ?? TestingStores.fakeLogStore(),
        ),
        screen.layer,
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        ...
        let fail = false;
        const logs = TestingStores.fakeLogStore({
          listRecent: () =>
...
const seeded = (): TestingStores.FakeActionStore => {
...
describe("run follow unhappy path", () => {
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        const screen = fakeRenderer();
        const stalled = TestingStores.fakeActionStore({ listActions: () => Effect.never });
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        ...
        const slow = TestingStores.fakeActionStore({
          listActions: () =>
