...
import * as RealSessions from "../src/sessions.ts";
...
describe("resume", () => {
  ...
  const live = (reporter: TestingReporter.Collector, minted: FakeMinted.FakeMinted) => {
    ...
    const sessions = RealSessions.Sessions.layer(4).pipe(
      Layer.provide(
        Layer.mergeAll(
          ...
          minted.layer,
          FakeQemu.fakeHost,
          TestingStores.fakeSessionStore().layer,
          TestingStores.fakeActionStore().layer,
          TestingStores.fakeDebugLogStore().layer,
          log,
