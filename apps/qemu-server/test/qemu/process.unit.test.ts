...
describe("spawnQemu happy path", () => {
  ...
  it.effect("keeps only the last 4096 bytes of stderr", () =>
    Effect.gen(function* () {
      ...
      const spawner = TestingSpawner.fakeSpawner(() => ({ stderr: `${noise}END` }));
      const process = yield* Process.spawnQemu(ARGS).pipe(Effect.provide(spawner.layer));
...
describe("createDisk", () => {
  ...
  it.effect("fails `qemu-img create exited <code>` on a non-zero exit", () =>
    Effect.gen(function* () {
      const spawner = TestingSpawner.fakeSpawner(() => ({ exitCode: 3 }));
  ...
  it.effect("fails with the spawn error when qemu-img cannot run", () =>
    Effect.gen(function* () {
      const spawner = TestingSpawner.fakeSpawner(() => ({ spawnError: "spawn qemu-img ENOENT" }));
      const error = yield* Effect.flip(
...
describe("createOverlay", () => {
  ...
  it.effect("fails `qemu-img create exited <code>` on a non-zero exit", () =>
    Effect.gen(function* () {
      const spawner = TestingSpawner.fakeSpawner(() => ({ exitCode: 1 }));
      const error = yield* Effect.flip(
...
describe("commandExists and displayHelp", () => {
  it.effect("is true when `command -v` exits 0 and false otherwise", () =>
    Effect.gen(function* () {
      const spawner = TestingSpawner.fakeSpawner((_, args) => ({
  ...
  it.effect("is false when the shell itself cannot be spawned", () =>
    Effect.gen(function* () {
      const spawner = TestingSpawner.fakeSpawner(() => ({ spawnError: "spawn /bin/sh ENOENT" }));
  ...
  it.effect("fails as a PlatformError when the binary cannot be spawned", () =>
    Effect.gen(function* () {
      const spawner = TestingSpawner.fakeSpawner(() => ({ spawnError: "spawn EACCES" }));
