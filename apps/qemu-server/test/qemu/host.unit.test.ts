...
const ALL_ENTRIES: TestingFs.Entries = {
  [Args.OVMF_CODE]: "File",
  [Args.OVMF_VARS]: "File",
...
const check = (display: Domain.QemuDisplay, fixture: Fixture = {}) =>
  Effect.gen(function* () {
    ...
    const spawner = TestingSpawner.fakeSpawner((command, args) => {
      if (command === "/bin/sh") {
    ...
    const fs = TestingFs.recordingFs(fixture.entries ?? ALL_ENTRIES);
...
describe("missingHostRequirements unhappy path", () => {
  ...
  it.effect("treats a failing -display help as a missing backend", () =>
    Effect.gen(function* () {
      ...
      const fs = TestingFs.recordingFs(ALL_ENTRIES);
