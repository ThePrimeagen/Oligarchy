...
const fixture = (options: Fixture = {}) =>
  Effect.gen(function* () {
    const spawner = TestingSpawner.fakeSpawner(
      TestingSpawner.byCommand({ [Args.QEMU_IMG]: options.qemuImg ?? { exitCode: 0 } }),
    );
    const fs = TestingFs.recordingFs(
      options.entries ?? { [FROM.disk]: "File", [FROM.vars]: "File" },
      {
        overrides: {
          rename: (from, to) =>
            Effect.suspend(() => {
              ...
              return options.renameFails?.includes(to) === true
                ? Effect.fail(TestingFs.permissionDenied("rename", to))
                : Effect.void;
            }),
        },
      },
...
describe("find", () => {
  it.effect("answers the two files beside the iso's path when both exist", () =>
    Effect.gen(function* () {
      ...
      expect(TestingFs.methods(fs)).toEqual(["stat", "stat"]);
    }),
  );
...
describe("save happy path", () => {
  ...
  it.effect("overwrites a minted disk already there without looking first", () =>
    Effect.gen(function* () {
      ...
      expect(TestingFs.methods(fs)).toEqual(["copyFile", "rename", "rename"]);
    }),
  );
});
