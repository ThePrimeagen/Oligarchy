...
type Options = {
  ...
  readonly sessionStore?: Parameters<typeof TestingStores.fakeSessionStore>[0];
  readonly actionStore?: Parameters<typeof TestingStores.fakeActionStore>[0];
  readonly debugLogStore?: Parameters<typeof TestingStores.fakeDebugLogStore>[0];
...
const harness = (options: Options = {}) => {
  const sessions = TestingStores.fakeSessionStore(options.sessionStore);
  const actions = TestingStores.fakeActionStore(options.actionStore);
  const debugLogs = TestingStores.fakeDebugLogStore(options.debugLogStore);
...
const texts = (h: Harness): ReadonlyArray<string> => TestingLog.texts(h.log);

const line = (h: Harness, prefix: string): TestingLog.Line | undefined =>
...
type SessionRow = TestingStores.FakeSessionStore["sessions"][number];
