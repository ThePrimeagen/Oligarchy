...
const layer = (
  ...
  // false is a missing file. undefined would take the default, which is a present file.
  ...
  Sessions.Sessions.layer(maxJobs, reserveQemu, relinquishQemu).pipe(
    ...
    // run reads oligarchy.json, so the caller has to have the file system too.
    Layer.provideMerge(configFs(config === false ? undefined : config)),
  );
...
const reservedRun = (ticket: string, prompt: string, action: "drive" | "mint" = "drive") =>
  Effect.gen(function* () {
    const sessions = yield* Sessions.Sessions;
    yield* sessions.reserve(ticket, action);
    return yield* sessions.run(ticket, prompt, RESULT);
  });

describe("Sessions.run happy path", () => {
  it.effect("launches ./driver with the action, the prompt and the result", () => {
    ...
    return Effect.gen(function* () {
      ...
      expect(spawner.spawned[0]?.args.join(" ")).not.toContain("--model");
  ...
  it.effect("a mint reserve launches ./driver as mint, with no model argument", () => {
    const spawner = FakeSpawner.fakeSpawner(() => ({ exitCode: 0 }));
    return Effect.gen(function* () {
      yield* reservedRun(TICKET, "do the work", "mint");
  ...
  it.effect(
    ...
    () => {
      ...
      return Effect.gen(function* () {
        ...
        yield* sessions.run(TICKET, "diagnose the session", RESULT);
        expect(spawner.spawned).toMatchObject([
          {
  ...
  it.effect("a diagnose with no oligarchy.json fails and spawns nothing (unhappy)", () => {
    const spawner = FakeSpawner.fakeSpawner(() => ({ exitCode: 0 }));
    return Effect.gen(function* () {
      const sessions = yield* Sessions.Sessions;
      yield* sessions.reserve(TICKET, "diagnose");
      const error = yield* Effect.flip(sessions.run(TICKET, "diagnose the session", RESULT));
      ...
      Effect.provide(
        layer(spawner, MAX_JOBS, qemuOk(), qemuRelinquishOk(), FakeLog.fakeLog(), false),
      ),
    );
  });
  ...
  it.effect("a diagnose that outlives the ceiling names opencode, not the driver", () => {
    ...
    return Effect.gen(function* () {
      ...
      const running = yield* Effect.forkChild(
        Effect.flip(sessions.run(TICKET, "diagnose the session", RESULT)),
      );
  ...
  it.effect(
    ...
    () => {
      ...
      return Effect.gen(function* () {
        ...
        const running = yield* Effect.forkChild(
          Effect.flip(sessions.run(TICKET, "do the work", RESULT)),
        );
        for (let i = 0; i < 100 && spawner.spawned[0] === undefined; i++) {
          yield* Effect.yieldNow;
        }
...
describe("Sessions.run unhappy path", () => {
  it.effect("forwards a spawn failure as RunFailed", () => {
    ...
    return Effect.gen(function* () {
      const sessions = yield* Sessions.Sessions;
      yield* sessions.reserve(TICKET, "drive");
      const error = yield* Effect.flip(sessions.run(TICKET, "do the work", RESULT));
      expect(error._tag).toBe("RunFailed");
...
describe("Sessions.abort happy path", () => {
  ...
  it.effect("kills only the matched ticket's CLI", () => {
    ...
    return Effect.gen(function* () {
      ...
      yield* sessions.reserve(TICKET, "drive");
      yield* sessions.reserve(OTHER, "drive");
      const first = yield* Effect.forkChild(sessions.run(TICKET, "first", RESULT));
      const second = yield* Effect.forkChild(sessions.run(OTHER, "second", RESULT));
...
describe("Sessions.abort unhappy path", () => {
  ...
  it.effect("a second run of the same ticket is a defect and leaves the first abortable", () => {
    ...
    return Effect.gen(function* () {
      ...
      yield* sessions.reserve(TICKET, "drive");
      const second = yield* Effect.forkChild(sessions.run(TICKET, "second", RESULT));
      for (let i = 0; i < 100 && spawner.spawned.length < 2; i++) {
        yield* Effect.yieldNow;
      }
...
describe("capacity", () => {
  ...
  it.effect(
    ...
    () => {
      ...
      return Effect.gen(function* () {
        ...
        yield* sessions.run(TICKET, "do the work", RESULT);
        expect(yield* sessions.jobs).toBe(0);
        expect(spawner.spawned).toHaveLength(1);
        expect((yield* Effect.flip(sessions.run(TICKET, "again", RESULT)))._tag).toBe("BadRequest");
      }).pipe(Effect.provide(layer(spawner, 1)));
    },
  );
  ...
  it.effect("run after reserve does not take a second slot", () => {
    ...
    return Effect.gen(function* () {
      const sessions = yield* Sessions.Sessions;
      yield* sessions.reserve(TICKET, "drive");
      const running = yield* Effect.forkChild(sessions.run(TICKET, "first", RESULT));
      for (let i = 0; i < 100 && spawner.spawned[0] === undefined; i++) {
        yield* Effect.yieldNow;
      }
      expect(spawner.spawned).toHaveLength(1);
      ...
      expect((yield* Effect.flip(sessions.run(OTHER, "second", RESULT)))._tag).toBe("BadRequest");
      yield* spawner.spawned[0]?.exit(0) ?? Effect.void;
      yield* Fiber.join(running);
    }).pipe(Effect.provide(layer(spawner, 1)));
  });
  ...
  it.effect("a run without a reservation is BadRequest and spawns nothing", () => {
    ...
    return Effect.gen(function* () {
      const sessions = yield* Sessions.Sessions;
      const error = yield* Effect.flip(sessions.run(TICKET, "first", RESULT));
      expect(error).toMatchObject({
        _tag: "BadRequest",
  ...
  it.effect("an aborted run frees its slot", () => {
    ...
    return Effect.gen(function* () {
      ...
      yield* sessions.reserve(OTHER, "drive");
      const next = yield* Effect.forkChild(sessions.run(OTHER, "second", RESULT));
      for (let i = 0; i < 100 && spawner.spawned.length < 2; i++) {
        yield* Effect.yieldNow;
      }
      expect(spawner.spawned).toHaveLength(2);
...
describe("QEMU-first reserve", () => {
  ...
  it.effect("a QEMU 503 takes no local slot and is AtCapacity", () => {
    ...
    return Effect.gen(function* () {
      ...
      expect((yield* Effect.flip(sessions.run(TICKET, "first", RESULT)))._tag).toBe("BadRequest");
      expect(spawner.spawned).toHaveLength(0);
    }).pipe(Effect.provide(layer(spawner, 1, reserveQemu)));
  });
...
describe("reservation expiry", () => {
  it.effect(
    ...
    () => {
      ...
      return Effect.gen(function* () {
        ...
        expect(yield* Effect.flip(sessions.run(TICKET, "late", RESULT))).toMatchObject({
          _tag: "BadRequest",
          message: "no reservation",
        });
        yield* sessions.reserve(OTHER, "drive");
  ...
  it.effect(
    ...
    () => {
      ...
      return Effect.gen(function* () {
        ...
        expect(givenBack).toEqual([TICKET]);
        expect(FakeLog.texts(log)).toEqual(["reservation expired; unused for 10 minutes"]);
        expect((yield* Effect.flip(sessions.run(TICKET, "late", RESULT)))._tag).toBe("BadRequest");
      }).pipe(Effect.provide(layer(spawner, 1, qemuOk(), relinquishQemu, log)));
    },
  );
  ...
  it.effect("expiry is per reservation: a younger one stays when an older one goes", () => {
    ...
    return Effect.gen(function* () {
      ...
      yield* sessions.run(OTHER, "still mine", RESULT);
      expect(spawner.spawned[0]?.command).toBe(OpenCode.BIN);
...
describe("one reserve in flight", () => {
  it.effect(
    "a second reserve on this server while one has not answered is AtCapacity and does not ask QEMU",
    () => {
      const qemu: Array<string> = [];
      return Effect.gen(function* () {
        ...
        yield* Effect.gen(function* () {
          const sessions = yield* Sessions.Sessions;
          const first = yield* Effect.forkChild(sessions.reserve(TICKET, "drive"));
          for (let i = 0; i < 100 && qemu.length < 1; i++) {
            yield* Effect.yieldNow;
          }
          expect(qemu).toEqual([TICKET]);
          const error = yield* Effect.flip(sessions.reserve(OTHER, "diagnose"));
          ...
          // The in-flight ticket has not been recorded, so this is not "already reserved".
  ...
  it.effect("a reserve that has answered does not block the next one, and both can run", () => {
    const spawner = FakeSpawner.fakeSpawner(() => ({}));
    return Effect.gen(function* () {
      const sessions = yield* Sessions.Sessions;
      yield* sessions.reserve(TICKET, "drive");
