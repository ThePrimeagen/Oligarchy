...
describe("dispatch unhappy path", () => {
  ...
  it.effect(
    "an Errored move that keeps failing is retried, then logged; the job stays errored",
    () =>
      Effect.gen(function* () {
        const refused = Errors.LinearError.make({
          operation: "moveToErrored",
          message: `linear: moving ${TICKET} to Errored failed`,
        });
        let attempts = 0;
        const fixed = harness(
        ...
        yield* start(fixed, http.layer);
        yield* eventually(
          () => fixed.log.lines.some((line) => line.text.startsWith("move to Errored failed")),
          "the Errored move given up",
        );
        expect(attempts).toBe(3);
        expect(fixed.automation.jobs[0]?.status).toBe("errored");
        expect(
          fixed.log.lines.find((line) => line.text.startsWith("move to Errored failed")),
        ).toMatchObject({
          level: "error",
          text: `move to Errored failed: linear: moving ${TICKET} to Errored failed`,
          agentId: TICKET,
          cause: refused,
        });
      }),
  ...
  it.effect("a prompt that cannot be rendered errors the claimed job and does not POST", () =>
    Effect.gen(function* () {
      const fixed = harness();
      seedResult(fixed.tests);
      seedJob(fixed.automation);
  ...
  it.effect(
    "a result with no Linear ticket errors after claim, moves nothing and does not POST",
    () =>
      Effect.gen(function* () {
        const fixed = harness();
...
describe("a running job left by the last automation server", () => {
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        ...
        expect(fixed.linear.calls).toEqual([
          cleared(TICKET),
          { method: "moveToErrored", identifier: TICKET, message: `drive errored; ${RESTARTED}` },
          cleared(TICKET_B),
        ]);
  ...
  it.effect(
    "a drive or mint whose result the driver closed has finished: it is closed completed, and nothing is sent, moved, or reported",
    () =>
      Effect.gen(function* () {
        const fixed = harness();
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        ...
        expect(moved(fixed.linear)).toEqual([
          { method: "moveToErrored", identifier: TICKET, message: `drive errored; ${RESTARTED}` },
          { method: "moveToErrored", identifier: TICKET_B, message: `drive errored; ${RESTARTED}` },
        ]);
      }),
  );
