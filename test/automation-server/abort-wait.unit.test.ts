...
import * as AbortWait from "../../src/automation-server/abort-wait.ts";
...
describe("abort wait", () => {
  it.effect("the frames are the braille spinner", () =>
    Effect.sync(() => {
      expect(AbortWait.SPINNER).toEqual(Follow.SPINNER);
    }),
  );
