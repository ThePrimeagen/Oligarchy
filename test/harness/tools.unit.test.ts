...
const ok = (body: { readonly args: ReadonlyArray<string> }): Tools.CommandLine => {
...
const body = (args: ReadonlyArray<string>) => ({ args });
...
describe("driver tools", () => {
  it("gives the model client.md and no ctrl tool", () => {
    ...
    expect(Object.keys(client.function.parameters.properties)).toEqual(["args"]);
  });

  it("runs the args the model sends as ./client", () => {
    expect(ok(body(["send-keys", "--agent-id", "OLI-1", "--keys", "hello world"]))).toEqual({
      bin: "./client",
      args: ["send-keys", "--agent-id", "OLI-1", "--keys", "hello world"],
    });
    expect(ok(body(["get-image", "--agent-id", "OLI-1", "-o", "shot.png"]))).toEqual({
      bin: "./client",
  ...
  it("refuses ctrl, session, and a payload that is not the client's arguments", () => {
    ...
    expect(refused("client", JSON.stringify({ args: ["get-image"], withImage: true }))).toContain(
      '["withImage"]',
    );
  });
});
