...
import * as Migrate from "@oligarchy/db/migrate";
...
Postgres.describeWithDatabase("database", () => {
  layer(Postgres.migratedLayer, { timeout: "60 seconds" })((scoped) => {
    ...
    scoped.effect(
      "ProcessStatsStore reports an automation-client, and deleting the servers row leaves the readings",
      () =>
        Effect.gen(function* () {
          const servers = yield* Servers.ServerStore;
          const store = yield* ProcessStats.ProcessStatsStore;
    ...
    scoped.effect(
      "ProcessStatsStore listSeries answers each name's newest readings oldest first, within count heartbeats of now and capped at count, qemu before automation-client then by name",
      () =>
        Effect.gen(function* () {
          const store = yield* ProcessStats.ProcessStatsStore;
          const database = yield* Client.Database;
          const suffix = uuid().slice(0, 8);
          const qemuName = `series-qemu-${suffix}`;
          const clientName = `series-client-${suffix}`;
          // Five readings, jobs counting them, stamped so that one is outside three heartbeats
          // (ninety seconds) and four are inside; the rank then keeps the newest three.
          const secondsAgo = [180, 80, 60, 40, 20];
          for (const [index, seconds] of secondsAgo.entries()) {
            const jobs = index + 1;
            yield* store.report(qemuName, "qemu", {
              jobs,
              memoryBytes: jobs * 100,
              cpuPercent: jobs * 10,
            });
            yield* database.run("stamp", (db) =>
              db.execute(
                sql`update process_stats set reported_at = now() - make_interval(secs => ${seconds}) where name = ${qemuName} and jobs = ${jobs}`,
              ),
            );
          }
          ...
          const series = yield* store.listSeries(3);
          const mine = series.filter((row) => row.name === qemuName || row.name === clientName);
          expect(mine).toEqual([
            {
              name: qemuName,
              type: "qemu",
              samples: [
                { jobs: 3, memoryBytes: 300, cpuPercent: 30 },
                { jobs: 4, memoryBytes: 400, cpuPercent: 40 },
                { jobs: 5, memoryBytes: 500, cpuPercent: 50 },
              ],
            },
            {
              name: clientName,
              type: "automation-client",
              samples: [{ jobs: 9, memoryBytes: 900, cpuPercent: 90 }],
            },
          ]);
          // A wider window takes the fourth reading too, and never the one three minutes back.
          const wider = yield* store.listSeries(4);
          expect(wider.find((row) => row.name === qemuName)?.samples.map((s) => s.jobs)).toEqual([
            2, 3, 4, 5,
          ]);
        }),
    );
    ...
    scoped.effect(
      "ProcessStatsStore listSeries answers nothing for a name whose readings are all older than the window (unhappy)",
      () =>
        Effect.gen(function* () {
          const store = yield* ProcessStats.ProcessStatsStore;
          const database = yield* Client.Database;
          const name = `stale-series-${uuid().slice(0, 8)}`;
          yield* store.report(name, "qemu", { jobs: 1, memoryBytes: 1, cpuPercent: 1 });
          yield* database.run("stamp", (db) =>
            db.execute(
              sql`update process_stats set reported_at = now() - interval '1 day' where name = ${name}`,
            ),
          );
          const series = yield* store.listSeries(240);
          expect(series.find((row) => row.name === name)).toBeUndefined();
        }),
    );
    ...
    scoped.effect(
      "ServerStore listMachines lists the qemu servers then the automation clients, each kind in registration order, with what each last said, its generation, its heartbeat and the database clock",
      () =>
        Effect.gen(function* () {
          const store = yield* Servers.ServerStore;
          ...
          expect(mine.map((row) => row.url)).toEqual([claimed, quiet, client]);
          const [announced, added, announcedClient] = mine;
          ...
          expect(announcedClient).toMatchObject({
            url: client,
            name: clientName,
            type: "automation-client",
            stats: first,
    ...
    scoped.effect("ServerStore heartbeat refuses a second url under the same name (unhappy)", () =>
      Effect.gen(function* () {
        const store = yield* Servers.ServerStore;
        const first = `http://10.0.0.40:${uuid().slice(0, 8)}`;
        const second = `http://10.0.0.41:${uuid().slice(0, 8)}`;
        const name = `taken-${uuid().slice(0, 8)}`;
        const stats: DbSchema.ServerStats = {
          qemus: 0,
          memory: { totalBytes: 1, usedBytes: 0 },
          cpu: { mean1m: 0, mean2m: 0, mean3m: 0 },
