...
`client experiment new` reads every test definition and creates a `test_runs` parent (ISO URL and server URL stored on the run) plus one pending `test_results` row per definition. The database generates the run UUID and each result's UUID primary key. Each definition gets its own Linear issue, labeled `agent test` and the run's version, whose body is `prompts/linear-issue.html` rendered with the issue identifier, the result UUID, the run UUID, the ISO URL, the server URL, the version, the definition, and `client.md` (see [cli.md](cli.md)). If any Linear call fails, the command closes the run and its results as failed with the API error, naming the issues already created.
...
- **info** — the default, and the normal story: the proxy listening, a session starting / running / stopped, an image served, serial served, chords sent, mouse sent, iso cache hits and downloads.
