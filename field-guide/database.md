...
`client test start` writes `test_results.session_id` and marks a pending row `running`. It takes the result UUID and the session UUID. The definition is already on the result. An unknown session, or a result that is missing or not pending, is a failure.
