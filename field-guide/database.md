...
`client experiment new` reads every test definition and creates a `test_runs` parent (ISO URL and server URL stored on the run) plus one pending `test_results` row per definition. The database generates the run UUID and each result's UUID primary key. Each definition gets its own Linear issue, which carries that result UUID, the run UUID, the ISO URL, the server URL, the definition details, and the labels `agent test` and the run's version. If Linear creation fails, the command closes the run and its results as failed with the API error.
