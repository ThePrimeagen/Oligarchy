...
Every command runs `./client` as a child process (`runClient`) with `--agent-id` and `--server-url` appended, so the REPL owns no HTTP of its own and the client's flag parsing and error rendering are the single source of truth. At `follow `, Tab runs `./ctrl session list --count 10 --json`, keeps its `running` and `downloading` rows, orders the running rows first, and opens a colored picker; up, down, Tab, and Shift-Tab move, Enter completes the session id, and Escape or Ctrl-C cancels. The picker calls downloading sessions `pending`, matching the follow view's status, and bounds its rows and text to the terminal. The REPL grammar is terse because a person types it; the action files translate it into the client's flags:
