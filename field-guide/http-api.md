...
The proxy defaults to `127.0.0.1:42069`. `./server <port>` binds that host on the given port and loads `.env`. The server takes no ISO. Bodies are JSON except the PNG and the serial log. Errors are `4xx`/`5xx` with `{"error": "<message>"}`.
...
`./server <port> --vga <card>` picks the guest VGA, independent of the host display. The default is `std`: QEMU's own card, no extra `-device`. That is the headless default and the card Quickshell cannot open (Bochs, no DRM render node). `virtio` is `-vga none -device virtio-vga` — still works with `-display none`, still capturable, and gives the guest a virtio-gpu DRM device. `virtio-gl` is `-vga none -device virtio-vga-gl` and needs a GL display (`egl-headless`, or `gtk`/`sdl`, which become `*,gl=on`). `-display none --vga virtio-gl` dies at QEMU start (`opengl is not available`). `-vga none` is only used when a virtio card replaces the console.

`./server <port> --automation` is the single flag that sets QEMU up for automation: `-display none` (no host window; QMP screendump still works) and `-vga none -device virtio-vga` (guest virtio-gpu, not `virtio-vga-gl`). `virtio-vga-gl` cannot run with `-display none` and breaks `/image`. `--automation` cannot be combined with `--display` or `--vga` — even `--automation --display none` is a startup error. Without `--automation`, `--display` still defaults to `none` and `--vga` to `std`. The proxy refuses to boot if qemu, qemu-img, OVMF, or the selected display backend is missing on the host.
