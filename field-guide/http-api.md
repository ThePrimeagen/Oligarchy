...
`./server <port> --automation` is the single flag that sets QEMU up for automation: `-display none` (no host window; QMP screendump still works) and `-vga none -device virtio-vga` (guest virtio-gpu, not `virtio-vga-gl`). `virtio-vga-gl` cannot run with `-display none` and breaks `/image`. `--automation` cannot be combined with `--display` or `--vga` — even `--automation --display none` is a startup error. Without `--automation`, `--display` still defaults to `none` and `--vga` to `std`.
