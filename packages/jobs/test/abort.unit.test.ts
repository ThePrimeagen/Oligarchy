...
describe("Abort.abort happy path", () => {
  it.effect(
    "aborts the pending drive, clears its ready label, then moves the ticket to Aborted",
    () =>
      Effect.gen(function* () {
        const h = H.harness();
...
describe("Abort.abort unhappy path", () => {
  it.effect(
    "a ticket with no job, or a job with nothing pending, is NoPendingAction and nothing moves",
    () =>
      Effect.gen(function* () {
        ...
        expect(missing).toEqual(
          JobsErrors.NoPendingAction.make({ ticket: H.TICKET, action: "drive" }),
        );
  ...
  it.effect(
    "a ticket that will not move after three attempts is one line and the row stays aborted",
    () =>
      Effect.gen(function* () {
  ...
  it.effect("a row write that fails is that error and nothing moves", () =>
    Effect.gen(function* () {
      ...
      H.seedResult(h.tests);
      const error = yield* Abort.abort(H.TICKET, "drive").pipe(
...
describe("Abort.running happy path", () => {
  it.effect(
    "closes the running drive aborted, clears its ready label, then moves the ticket to Aborted",
    () =>
      Effect.gen(function* () {
        const h = H.harness();
